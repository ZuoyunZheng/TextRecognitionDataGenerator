# Usage
1. Clone https://github.com/ZuoyunZheng/TextRecognitionDataGenerator and install via  `pip install .`
2. Download lvis http://images.cocodataset.org/zips/val2017.zip
3. Download google-fonts https://github.com/google/fonts/archive/main.zip (~2G) and filter using the filtering script `python scripts/filter_fonts.py --source <google_fonts_dir> --output <custom_fonts_dir>`.
4. Reproduce trdg10
   ```
   trdg -rs -let -num -sym -f 64 -t 4 -k 5 -rk -obb 1 -d 4 -do 2 -al 3 -tc "#000000,#FFFFFF" -cs 5 -m 2,2,2,2 -rm -fi -fd <custom_fonts_dir> -id <lvis_dir> -stw 3 -stf "#000000,#FFFFFF" -rst --output_dit <dataset_dir> -c 5000000 -or 0 -b 3
   ```
  + It is to be noted, the current trdg10 was generated before random-seed-fixing was added, so it **cannot** be 1-to-1 reproduced. However, as of now, trdg has random-seed-fixing implemented, new generated dataset are **completely deterministic** and can be reproduced reliably.
<hr \>
